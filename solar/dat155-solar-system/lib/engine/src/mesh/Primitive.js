...
export default class Primitive {
    ...
    */
    ...
    static createRing(material, tesselation = 100, rad = 0, mode) {
        const vertices = [];
        const normal = [0, -1, 0,];
        const normals = [];
        const uvs = [];

        for (let t = 0; t < tesselation; t++) {
            let theta1 = ((t+1)/tesselation) * 2 * Math.PI;
            let theta2 = (t/tesselation) * 2 * Math.PI;
            ...
            let x3 = rad * Math.cos(theta1);
            let y3 = rad * Math.sin(theta1);
            let x4 = rad * Math.cos(theta2);
            let y4 = rad * Math.sin(theta2);
