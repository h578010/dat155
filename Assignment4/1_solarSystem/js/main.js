...
import { Renderer, Scene, Node, Mesh, Primitive, PhongMaterial, BasicMaterial, CubeMapMaterial, PerspectiveCamera, vec3, vec4, Light } from '../lib/engine/index.js';
...
const ring2Primitive = Primitive.from(ringPrimitive, ringMaterial);
...
const ringsNode = new Node(saturnCenterNode);
const ring1Node = new Node(ringsNode);
const ring2Node = new Node(ringsNode);
...
ring2Node.setTranslation(0, -0.001, 0);
...
const ring1 = new Mesh([ringPrimitive]);
const ring2 = new Mesh([ring2Primitive]);
...
ringsNode.setScale(1.4, 1.4, 1.4);
ringsNode.rotateX(3.14/6);
